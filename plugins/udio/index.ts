...
async function searchMusic(query, page) {
  const pageSize = 30;
  const data = `{"searchQuery":{"sort":"plays","searchTerm":"${query}"},"pageParam":${
    page - 1
  },"pageSize":${pageSize},"trendingId":"93de406e-bdc1-40a6-befd-90637a362158"}`;
  ...
  const result = (await axios(config)).data.data;
...
async function getTopListDetail(topListItem) {
  if (topListItem.type === "playlist") {
    const res = (
      await axios.get(
        `https://www.udio.com/api/playlists?id=${topListItem.id}`,
        {
          headers,
        }
      )
    ...
    const songs = (
      await axios.get(`https://www.udio.com/api/songs?songIds=${songList}`, {
        headers,
      })
    ).data.songs;
    ...
    const pageSize = 30;
    const data = `{"searchQuery":{"sort":"plays","searchTerm":""${
      topListItem.maxAgeInHours
    ...
    const songs = (await axios(config)).data.data;
